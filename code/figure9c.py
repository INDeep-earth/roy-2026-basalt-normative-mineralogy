"""
Figure 9c plotting script for Roy et al. (2026)

Purpose
-------
Plots MAGEMin model output from Figure 9c on the unfolded basalt tetrahedron.
...
Notes
-----
- Points are coloured by pressure P[kbar].
- Data are projected into the Ne–Ol–Di, Ol–Di–Hy, and Di–Hy–Qz fields
  according to normative_name.
- The colour scale is determined from the data range in the sheet.
"""
...
# ---------------------------------------------------------------------
INPUT_FILE = Path(
    r"C:\Users\N11564083\OneDrive - Queensland University of Technology\Documents\1_PhD\Chapter 1\Ms progress\Submission Documents\Supplementary Files\Supplementary File 3 - MAGEMin Output Files.xlsx"
)
SHEET_NAME = "Fig 9c"

OUTPUT_DIR = Path(r"C:\add\your\output-path\test")
OUTPUT_FILE = OUTPUT_DIR / "figure9c_magemin_projection.pdf"
...
def ternary_to_cartesian(
    ...
    """
    Convert ternary proportions to Cartesian coordinates using barycentric
    interpolation.
    """
...
def main() -> None:
    ...
    col_p = get_col(df, "P[kbar]", ["P[kbar]", "P [kbar]", "Pressure", "Pressure_kbar"])
    ...
    df = df[[col_norm, col_ne, col_ol, col_di, col_hy, col_qz, col_p]].copy()
    df.columns = [
        "normative_name",
        "Nepheline_%wt",
        "Olivine_%wt",
    ...
    # Clean data
    df = df.dropna(subset=["normative_name"]).copy()
    numeric_cols = [
        "Nepheline_%wt",
        "Olivine_%wt",
        "Diopside_%wt",
    ...
    df_left = df[df["normative_name"].astype(str).str.strip().str.lower() == "nepheline_normative"].copy()
    df_middle = df[df["normative_name"].astype(str).str.strip().str.lower() == "olivine_normative"].copy()
    df_right = df[df["normative_name"].astype(str).str.strip().str.lower() == "quartz_normative"].copy()
    ...
    df_all = pd.concat([x for x in [df_left, df_middle, df_right] if not x.empty], ignore_index=True)

    if df_all.empty:
        raise ValueError("No plottable data remain after processing normative groups.")

    # Dynamic colour scale from data
    p_vals = df_all["P[kbar]"].to_numpy()
    vmin = float(np.nanmin(p_vals))
    vmax = float(np.nanmax(p_vals))
    ...
    # Apex labels
    ax.text(ne[0], ne[1] + 0.05, "Ne", fontsize=12, ha="center", va="center", fontweight="bold")
    ax.text(ol[0], ol[1] - 0.05, "Ol", fontsize=12, ha="center", va="top", fontweight="bold")
    ax.text(di[0], di[1] + 0.05, "Di", fontsize=12, ha="center", va="center", fontweight="bold")
    ax.text(hy[0], hy[1] - 0.05, "Hy", fontsize=12, ha="center", va="top", fontweight="bold")
    ax.text(qz[0], qz[1] + 0.05, "Qz", fontsize=12, ha="center", va="center", fontweight="bold")
    ...
    ax.set_title("Figure 9c", fontsize=13)

    cbar = plt.colorbar(sc, ax=ax, pad=0.02)
    cbar.set_label("P [kbar]")
